...
import { normalizeNationalityFields, normalizeNationalityValue } from "@/lib/nationality";
import type { PowerCandidate } from "@/lib/power-parties";
...
interface FormData {
  ...
  seller_rep_candidates: PowerCandidate[];
  // Buyer
  ...
  buyer_rep_candidates: PowerCandidate[];
  // Vehicle
...
const defaultForm: FormData = {
  ...
  seller_rep_candidates: [],
  buyer_full_name: "",
  buyer_ci: "",
  buyer_nationality: "oriental",
  buyer_birth_date: "",
  buyer_birth_place: "",
  ...
  include_notarial_certification: true,
};
...
function normalizeInitialFormData(initialData?: Partial<FormData>): Partial<FormData> | undefined {
  if (!initialData) return undefined;
  return normalizeNationalityFields(initialData as Record<string, unknown>, nationalityFieldKeys) as Partial<FormData>;
}
...
export default function TransactionForm({
  ...
  const normalizedInitialData = normalizeInitialFormData(initialData);
  const [form, setForm] = useState<FormData>(
    normalizedInitialData ? { ...defaultForm, ...normalizedInitialData } : defaultForm
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [validationErrors, setValidationErrors] = useState<ValidationErrors>({});
  const previousPriceSyncRef = useRef({
    amount: normalizedInitialData?.price_amount ?? defaultForm.price_amount,
    currency: normalizedInitialData?.price_currency ?? defaultForm.price_currency,
  });
  ...
  useEffect(() => {
    const previous = previousPriceSyncRef.current;

    setForm((prev) => {
      const shouldSyncWords =
        !prev.price_in_words.trim() ||
        isAutoGeneratedPriceInWords(prev.price_in_words, previous.amount, previous.currency);

      if (!shouldSyncWords) return prev;

      const nextWords = amountInWords(prev.price_amount);
      if (prev.price_in_words === nextWords) return prev;

      return {
        ...prev,
        price_in_words: nextWords,
      };
    });

    previousPriceSyncRef.current = {
      amount: form.price_amount,
      currency: form.price_currency,
    };
  }, [form.price_amount, form.price_currency]);
  ...
  function getRepresentativeCandidates(prefix: "seller" | "buyer") {
    return (prefix === "seller" ? form.seller_rep_candidates : form.buyer_rep_candidates) ?? [];
  }

  function getRepresentativeIdentifier(candidate: PowerCandidate) {
    return candidate.kind === "company" ? candidate.rut : candidate.ci_number;
  }

  function getSelectedRepresentativeCandidate(prefix: "seller" | "buyer") {
    const candidates = getRepresentativeCandidates(prefix);
    const currentName = prefix === "seller" ? form.seller_rep_name : form.buyer_rep_name;
    const currentIdentifier = prefix === "seller" ? form.seller_rep_ci : form.buyer_rep_ci;

    return candidates.find((candidate) => {
      const candidateIdentifier = getRepresentativeIdentifier(candidate) ?? "";
      return (currentIdentifier && candidateIdentifier === currentIdentifier) ||
        (currentName && candidate.full_name === currentName);
    }) ?? null;
  }

  function applyRepresentativeCandidate(prefix: "seller" | "buyer", candidateId: string) {
    const candidates = getRepresentativeCandidates(prefix);
    const candidate = candidates.find((item) => item.id === candidateId) ?? null;
    const nameKey = `${prefix}_rep_name` as keyof FormData;
    const ciKey = `${prefix}_rep_ci` as keyof FormData;
    const addressKey = `${prefix}_rep_address` as keyof FormData;
    const identifier = candidate ? getRepresentativeIdentifier(candidate) ?? "" : "";

    set(nameKey, ((candidate?.full_name ?? "") as FormData[typeof nameKey]));
    set(ciKey, (identifier as FormData[typeof ciKey]));
    set(addressKey, ((candidate?.address ?? "") as FormData[typeof addressKey]));
  }
  ...
  function buildClientPayload(prefix: "seller" | "seller2" | "buyer" | "buyer2") {
    ...
    const base: Record<string, unknown> = {
      ...
      nationality: normalizeNationalityValue(form[p("nationality")]),
  ...
  function buildTransactionPayload(status: string, sellerId: string, seller2Id: string | null, buyerId: string, buyer2Id: string | null, vehicleId: string, matrizNumber: number | null, folioStart: number | null) {
    return {
      ...
      include_notarial_certification: form.include_notarial_certification,
        ...
        {form[p("is_company")] ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
            ...
            <Input
              ...
              onChange={(v) => set(p("nationality"), normalizeNationalityValue(v))}
            />
            <Input
  ...
  function renderRepresentativeSection(
    ...
    const candidates = getRepresentativeCandidates(prefix);
    const selectedCandidate = getSelectedRepresentativeCandidate(prefix);
    const selectedCandidateId = selectedCandidate?.id ?? "";
    const identifierLabel = candidates.some((candidate) => candidate.kind === "company") ? "CI / RUT del apoderado" : "CI del apoderado";
    const candidateOptions = [
      { value: "", label: "Seleccionar..." },
      ...candidates.map((candidate) => ({
        value: candidate.id,
        label: candidate.full_name ?? "Sin nombre",
      })),
    ];
    const identifierOptions = [
      ...
      ...candidates.map((candidate) => ({
        value: candidate.id,
        label: getRepresentativeIdentifier(candidate) ?? "Sin documento",
      })),
    ];
            ...
            {candidates.length > 0 ? (
              <>
                <Select
                  ...
                  onChange={(value) => applyRepresentativeCandidate(prefix, value)}
    ...
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <Section title="Protocolización y papel notarial" defaultOpen={false}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="md:col-span-3">
            <Checkbox
              label="Incluir CERTIFICO QUE, protocolización y testimonio al final"
              checked={form.include_notarial_certification}
              onChange={(v) => set("include_notarial_certification", v)}
            />
            <p className="mt-1 text-xs text-gray-500">
              Si lo desmarcás, el documento cierra con líneas para firma como el modelo manual de Franco.
            </p>
          </div>
        </div>
      </Section>
